...
- REST API endpoints for fetching recommendations:
  - `GET /products/{productId}/similar` - Attribute-based recommendations
  - `GET /products/{productId}/frequently-bought-with` - Association rule recommendations
  - `GET /users/{userId}/recommended` - Personalized collaborative filtering
  - `GET /products/{productId}/recommendations?userId={userId}` - Hybrid context-aware
  - `GET /debug/recommendations/:productId?explain=true` - Debug endpoint with score breakdown
...
- [ ] **Task 1.4**: Create environment configuration
  ...
  - Action: Define env vars (MONGODB_URI, REDIS_URL, PORT, NODE_ENV)
  - Notes: Use dotenv for loading, Zod for validation
...
- [ ] **Task 3.3**: Implement Redis client with Sentinel/Cluster support
  - File: `src/storage/redis-client.ts`
  - Action: Create RedisClient class with connection management, circuit breaker
...
- [ ] **Task 4.1**: Implement feature extraction and normalization
...
- [ ] **Task 4.4**: Implement item-based collaborative filtering
  ...
  - Notes: Filter products with <10 orders (adaptive threshold logic), handle sparse data
...
- [ ] **Task 6.1**: Setup BullMQ queue and workers
  - File: `src/jobs/queue-manager.ts`
  - Action: Initialize BullMQ with Redis, create queues for content/collaborative/association jobs
...
- [ ] **Task 6.5**: Implement transactional batch executor
  - File: `src/jobs/batch-executor.ts`
  - Action: Wrap batch jobs with transactional logic: temp version → validate → atomic promotion → cleanup
...
- [ ] **Task 7.2**: Implement recommendation routes
  - File: `src/api/routes/recommendations.ts`
  - Action: Create GET endpoints: /products/:id/similar, /products/:id/frequently-bought-with, /users/:id/recommended, /products/:id/recommendations
  - Notes: Validate params with Zod, handle errors gracefully, return provenance data

- [ ] **Task 7.3**: Implement debug endpoint
  - File: `src/api/routes/debug.ts`
  - Action: Create GET /debug/recommendations/:id?explain=true with full score breakdown
  - Notes: Include batch metadata, cache hit status, computation time
...
- [ ] **Task 8.2**: Setup Prometheus metrics
...
- [ ] **Task 10.5**: Write deployment guide
  - File: `docs/deployment.md`
  - Action: Document deployment options (Docker, K8s), scaling strategy, monitoring setup
...
- [ ] **AC4**: Given a valid productId and userId, when GET /products/{productId}/recommendations?userId={userId} is called, then returns hybrid recommendations with context-aware weight blending
...
- [ ] **AC29**: Given debug endpoint is called with ?explain=true, when response is returned, then includes full score breakdown and batch metadata
...
- [ ] **AC32**: Given recommendation quality degrades, when offline metrics show Precision@10 <0.5, then alert is triggered
